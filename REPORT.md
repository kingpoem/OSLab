...
2. 每种配置下重新编译 `libmy_vm.a`；
3. 用相同的源码（`benchmark/test.c`、`benchmark/multi_test.c`、`tools/tlb_bench.c`）
   分别链接执行，每次运行的 stdout+stderr 写入 `logs/<bench>_<cfg>.log`；
4. 解析 `printTLBStats()` 输出，汇总到 `logs/summary.csv`；
5. 退出时无条件恢复原 `defines.h`（trap）。

```bash
# 运行全部默认矩阵 (10 组配置 × 3 程序 = 30 次运行)
...
默认配置 (`PAGE_SIZE=4096`, `TLB_SIZE=32`, `PM_SIZE=64MB`) 下完整输出
（来自 `logs/test_p4096_t32_pm64.log`）：
...
- 三次 `myMalloc(6000)` 返回 `0x1000 / 0x3000 / 0x5000`：每次 6 KB > 4 KB 占 2 页，
  连续虚拟页号正确递增；
- 写入 1 → 读出 1，矩阵乘后每个元素为 10（`SIZE=10` 维全 1 矩阵自乘）；
- `myFree` 后再次 `myMalloc(6000)` 复用了 `0x1000`，命中 `free function works`。

### 2.2 多线程基准 `benchmark/multi_test.c`

15 线程并发分配/写入/矩阵乘/释放，输出（`logs/multi_p4096_t32_pm64.log`）：
...
`vm_lock` 与 `tlb_lock` 两把互斥锁保证并发安全；所有配置下退出码均为 0。
分配地址中混入了 `0x1d000` 是因为多线程中分配/释放的交错导致虚拟位图扫描跳过了
被某线程暂时占用的中间区段，符合 FIFO 分配的预期。
...
在 `initMemoryAndDisk()` 中按 `defines.h` 推导地址布局：
...
均使用 `PAGE_SIZE` / `g_vm.offset_bits` / `g_vm.num_pte_entries` 这些运行时常量，
因此**只需修改 `defines.h` 重新编译即可切换页面大小**，无需改动逻辑。
...
> 访问总数固定为 188540 是因为 `tlb_bench` 三种模式的迭代次数是写死的；
> 实际命中/缺失差异由 `PAGE_SIZE` 与 `TLB_SIZE` 决定。
...
`tlb_bench` 总分配 < 8 MB，未触及 swap；本场景 swap 路径未激活。
单独的 swap 压力测试（参见 step6 提交说明，75 MB 写入 vs 64 MB 物理内存）
证明 `pageFault()` 换出 / 换入路径正确。
